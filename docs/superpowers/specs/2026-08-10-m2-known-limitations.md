...
|---|---|
| A code comment on the close-ownership fix (§7.4 below) claims Bun's `net.Server` does not unlink a unix socket file on `close()`, making this project's own `unlinkSync` "the only" one. The final whole-branch re-review verified this is factually false — Bun unlinks by path synchronously inside `close()`, deleting whatever currently answers to that name. | Doesn't change correctness: the actual invariant that makes the fix safe is different and real — every replacement server bind is always preceded by closing the predecessor first, everywhere `createMcpServer` is called, so Bun's own unlink never fires twice on the same name. The comment states the wrong reason for a right fix; worth correcting so a future maintainer doesn't rely on the stated (false) reasoning if that invariant is ever relaxed. |
| `MessageBus.deliver(messageId)` / `MessageRepo.markDelivered(id)` (singular) are now genuinely dead code — the live path is `deliverAll`/`markDeliveredMany`, added when `cw_inbox` was fixed to actually ack what it returns (§7.1 below). Only referenced by their own unit tests. | Left in place per this project's dead-code convention (mention, don't delete opportunistically) rather than removed as a drive-by cleanup during the fix wave that found them. Candidate for deletion in a future pass. |
