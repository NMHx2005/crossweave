...
**Horizon D is wired** (ab8ed61→92bbafd): CI `sandbox-linux` job (ubuntu-latest + bubblewrap) proves `buildBwrapArgs` escape table; `session.data` E2E helpers `src/gateway/e2e.ts` (HKDF + aes-256-gcm via node:crypto, no native) — relay `src/gateway/relay.ts` stays dumb forwarder, ends enforce allowlists. Relay deploy + workspace routing remain deferred. Horizon E adds `workspace.openFile` READ (path containment, 512k cap) + web client `openFile`; Horizon F adds `bwrap` check to `install.sh`.
